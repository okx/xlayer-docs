...
- 1.x L1 deposit → credit on L2 (bridge in)
  ...
  - 1.2 op-node monitors L1 contract events and derives deposit transactions
  - 1.3 op-node includes deposit transaction in L2 block via Engine API
  - 1.4 op-geth executes the deposit and credits user account
  - 1.5 The Sequencer continues producing L2 blocks
