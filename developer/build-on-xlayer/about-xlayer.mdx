...
## Consensus & finality
X Layer uses a PoE‑based consensus on L2. Transactions are executed by the EVM and finalized on L2 by the trusted sequencer set. Cross‑chain最终性由 Polygon 侧的 `agglayer` 子系统完成：xlayer 生成并存储证书数据（metadata/epochs 等），`aggsender` 同步并提交证书到 `agglayer`，随后由 `agglayer`/`agglayer‑prover` 在 L1 提交 PP 证明。两类关键流程分别是：**L2 交易出块与数据写入** 和 **跨链证书 + PP 提交**。参与者包括 **Sequencer** 与 **aggsender/agglayer** 链路。

**Sequencer**: 负责在 L2 打包交易、生成区块并写入链/DS 数据。

**aggsender/agglayer**: `aggsender` 从节点侧获取区块/证书并做双检（Double Check），将证书同步到 `agglayer`；`agglayer`（配合 `agglayer‑prover`）在 L1 提交 PP 证明并完成跨链结算。
