...
Below is a high‑level view of components and the cross‑chain flow in PP mode:

1. A user deposits or initiates a bridge action on L1 (bridge contract emits events).
2. The Bridge Service in the X Layer deployment monitors L1 contract events and prepares the corresponding L2 actions.
3. Erigon RPCs, Pool Manager and the Erigon Sequencer handle L2 transactions/blocks. The sequencer includes the bridge claim/mint in an L2 block and writes the state to the storage backend (e.g., Postgres/SQLite).
4. Waypoints and L2 block information are synced and indexed; proxies relay user transactions to RPCs.
5. On the Polygon side, `agglayer` (and `agglayer-prover`) syncs L1 information, generates a PP certificate and public inputs.
6. The certificate proof is committed to L1 (verifier/bridge contract). After verification, the cross‑chain operation (e.g., withdrawal) is finalized on L1.
