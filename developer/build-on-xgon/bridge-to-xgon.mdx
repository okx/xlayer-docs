...
The ZkEVM bridge contract allows ETH and ERC20 token bridging from L1 to L2 using the bridgeAsset function. Notice that ERC20 tokens will have a different address on L2. You can use the precalculatedWrapperAddress function to query the new address in ZkEVMBridgeContract.sol contract
#### bridgeAsset
...
### Depositing OKB and ERC-20 tokens from L2
The same bridge contract is used on both L1 and L2, so the interface definition is identical.
Note: Claiming assets on Ethereum will consume gas fees and is not free.