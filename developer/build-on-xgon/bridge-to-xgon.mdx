# Bridge to XGON
You can use [ZkEVM Bridge Contract](https://github.com/okx/xgon-contracts/blob/release/v0.1.0/contracts/PolygonZkEVMBridge.sol "ZkEVM Bridge Contract") to bridge assets like ETH, OKB, and ERC-20 tokens between L1 and L2. However, there are a few points to note:：
- ETH is the native token of L1 (Ethereum). After bridging to L2 XGON, it becomes a Wrapped ETH (WETH) ERC-20 token.
- OKB is an ERC-20 token of L1 (Ethereum). After bridging to L2 XGON, it becomes a native token.
...
The ZkEVM bridge contract allows ETH and ERC-20 token bridging from L1 to L2 using the bridgeAsset function. Notice that ERC-20 tokens will have a different address on L2. You can use the precalculatedWrapperAddress function to query the new address in `[ZkEVMBridgeContract.sol](https://github.com/okx/xgon-contracts/blob/release/v0.1.0/contracts/PolygonZkEVMBridge.sol "ZkEVMBridgeContract.sol")` contract
