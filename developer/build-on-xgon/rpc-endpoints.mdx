# RPC endpoints
Xgon supports JSON-RPC methods that are compatible with Ethereum, and zkEVM RPC methods for zkEVM.

## Mainnet (chain-ID: 0xxx, xxx in decimals)
https://xxxx.Xgonhain.org/ (coming soon)
## Testnet (chain-ID: 0xxx, xxx in decimals)
https://xxx.Xgonhain.org/ (coming soon)

## Prerequisite readings
Before you begin using RPC endpoints on Xgon, it's important to read the relevant articles for guidance:
...
```
# mainnet
curl -X POST --data '{"jsonrpc":"2.0","method":"eth_blockNumber","params":[],"id":1}' -H "Content-Type: application/json" https://xxxx.Xgonhain.org/
# testnet
curl -X POST --data '{"jsonrpc":"2.0","method":"eth_blockNumber","params":[],"id":1}' -H "Content-Type: application/json" https://xxxx.Xgonhain.org/
```
## JSON-RPC methods
There are two types of RPC methods on Xgon, one is Ethereum compatible RPC, and the other one is zkEVM RPC. Each RPC offers specific functionalities.
