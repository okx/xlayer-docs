...
1. `datadir` — specify your desired data directory path. This is the directory where the snapshot (described below) should be extracted to.
...
3. Extract to `datadir`:
```bash
mkdir -p ${your-data-dir}
tar -xzf xlayer-erigon-mainnet-snap-*.tar.gz -C ${your-data-dir}
# After extraction, ${your-data-dir} becomes your node data directory
```
4. Start the node using the command below. The client will continue syncing from the snapshot state.

Notes:
- Do not set `datadir` to the compressed file path. It must point to the directory created after extraction.
