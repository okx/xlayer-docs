...
> **💡 Recommended**: For most users, we recommend using the [one-click setup script](./setup-rpc.mdx) which automatically handles snapshot download and extraction. This page is intended for advanced users who need to manually download snapshots (e.g., for data recovery or custom setups).
