...
- You can also find ABI on [OKX Explorer](https://www.okx.com/explorer/ "OKX Explorer"), but this is not always reliable because the ABI uploaded there may be out of date. Make sure you have the correct ABI or your subgraph will fail to run.
...
We use the [Graph Typescript Library](https://github.com/graphprotocol/graph-tooling "Graph Typescript Library") which contains utilities for interacting with the Graph Node store and conveniences for handling smart contract data and entities. You can use this library in your mappings by importing `@graphprotocol/graph-ts` into `mapping.ts`.
