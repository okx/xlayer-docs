...
## Aggregating batches
Once the aggregator node possesses the proof, it invokes the [ZkEVM.sol](https://github.com/okx/x1-contracts/blob/release/v0.1.0/contracts/PolygonZkEVM.sol "ZkEVM.sol") smart contract's `trustedVerifyBatches` function and provides the received proof. Here's the function's source code:
```solidity
...
```
Regarding the `inputSnark` section, please refer to the [**Transaction validation**](/developer/learn-more/x1-protocol/transaction-validation "Transaction validation") chapter for more details.