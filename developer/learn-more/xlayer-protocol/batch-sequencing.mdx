...
- It must include a `globalExitRoot` value that is present in the **GlobalExitRootMap** of the bridge's L1 [ZkEVMGlobalExitRoot.sol](https://github.com/okx/xlayer-contracts/blob/release/v0.1.0/contracts/PolygonZkEVMGlobalExitRoot.sol "ZkEVMGlobalExitRoot.sol") contract. A batch is valid only if it includes a valid `globalExitRoot`.
