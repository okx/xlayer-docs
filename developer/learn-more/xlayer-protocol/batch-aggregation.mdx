...
## Aggregating batches
Once the aggregator node possesses the proof, it invokes the [ZkEVM.sol](https://github.com/okx/xlayer-contracts/blob/release/v0.1.0/contracts/PolygonZkEVM.sol "ZkEVM.sol") smart contract's `trustedVerifyBatches` function and provides the received proof. Here's the function's source code:
```solidity
