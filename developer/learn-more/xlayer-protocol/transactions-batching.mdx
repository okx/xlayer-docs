...
The trusted sequencer must batch the transactions using the following `BatchData` struct specified in the [ZkEVM.sol](https://github.com/okx/xlayer-contracts/blob/release/v0.1.0/contracts/PolygonZkEVM.sol "ZkEVM.sol") contract:
```solidity
struct BatchData {
        bytes transactions;
