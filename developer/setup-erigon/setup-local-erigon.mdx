...
- Wallet / dApp front‑end (MetaMask, Rabby): Settings → Networks → Add Network (paste values) → Save
- Dev environment (Hardhat, Foundry, Truffle): Add to project config and restart your tests/scripts
- CLI scripts (Curl/wget/Postman/shell-scripts): Point the command at the RPC URL

### Example: MetaMask (Mainnet)
```json
{
...
### Example: Foundry endpoint
```toml
[rpc_endpoints]
xlayer = "https://rpc.xlayer.tech"
```

### 10‑second CLI smoke test
```bash
curl -X POST \
...
```
If you get a hex block number back, the endpoint is live.

---

## Run Your Own X Layer RPC Node (xlayer‑erigon)
Running your own node removes the public rate limit (100 requests/sec per IP on mainnet/testnet) and gives you full control over logs/metrics and debug/trace features.

### When should you self‑host?
- Push >50 RPS or run long‑running indexers
- Need deterministic uptime/compliance
- Require full logs & `trace`/`debug` support

Stick to public RPC if you only test or run low‑traffic dApps.
...
- L1 Endpoint: An Ethereum Mainnet RPC (Alchemy/Infura/self‑hosted geth) used for L2 synchronization
...
## Multi‑step install & initialization (Erigon setup)
Reference: Deploy/mainnet/setup-erigon-rpc.md at `okx/Deploy`
...
### 2) Create mainnet config
Create `xlayerconfig-mainnet.yaml` in the repo root and fill it as below. Update:
- `datadir` → your data directory path
- `zkevm.l1-rpc-url` → your L1 (Ethereum mainnet) RPC URL
