...
This contract receives L1 USDC from users, and triggers the ZkMinterBurner contract on zkEVM (through the Polygon ZkEVM Bridge) to mint USDC-e. It holds all of the L1 backing of USDC-e. It's also triggered by the Bridge to withdraw L1 USDC.

4. [ZkMinterBurner](https://zkevm.polygonscan.com/address/0xBDa0B27f93B2FD3f076725b89cf02e48609bC189 "ZkMinterBurner") (zkEVM) <br/>
This contract receives BridgeWrappedUSDC on zkEVM and mints back USDC-e. It also has a permissionless publicly callable function called "migrate" which withdraws all BridgedWrappedUSDC to L1 through the Bridge. The beneficiary address is the L1Escrow, thus migrating the supply and settling the balance.

## Bridge transactions guide

Bridge USDC(L1) to USDC-e(L2)

1. Approve USDC amount for L1Escrow on L1 using our [bridge](https://www.okx.com/xlayer/bridge "xlayer bridge").
2. bridgeToken on L1.
3. claimMessage on L2.

Bridge USDC-e(L2) to USDC(L1)

1. Approve USDC-e amount for ZkMinterBurner our [bridge](https://www.okx.com/xlayer/bridge "xlayer bridge").
2. bridgeToken on L2.
3. claimMessage on L1.

Bridge USDC(L1) to BWUSDC(L2)

1. Approve USDC amount for ZkEVMBridge on L1.
2. bridgeAsset on L1.
3. claimAsset (BWUSDC) on L2.

Convert BWUSDC(L2) to USDC-e(L2)

1. Approve BWUSDC amount for NativeConverter on L2.
2. Convert on L2.

Migrate BWUSDC(L2) to USDC(L1)
...
``` 
    function bridgeToken(
        address destinationAddress, //indicate your address on L2
        uint256 amount, //amount of usdc
        bool forceUpdateGlobalExitRoot //true
    ) external;
```
   - To: L1Escrow Address on L1
   - Description: Transfers USDC to L1Escrow and sends a bridgeMessage to the ZkEVMBridge on L2.
...
```
   - To: ZkMinterBurner Address on L2
...
```
   - To: USDC-e / BWUSDC Address on L2
...
```
   - To: NativeConverter Address on L2
...
3. Samples of BridgeEvent:
    - Emitted from a Testnet L1 to L2 bridgeToken [transaction](https://sepolia.etherscan.io/tx/0xcec854f18b42d9d4600dcbe5c5bc699fb4241f896d5bc5c6ffec2cb882025664#eventlog "bridgeToken transaction").
...
2. Example requests
      - Testnet
    - Intent: Claiming a bridgeToken call sent from L1 with a depositCount of 1103532 
      - https://testrpc.x1.tech/priapi/v1/ob/bridge/merkle-proof?net_id=0&deposit_cnt=1103532
    - Intent: Claiming a bridgeToken call sent from L2 with a depositCount of 776346
      - https://testrpc.x1.tech/priapi/v1/ob/bridge/merkle-proof?net_id=1&deposit_cnt=776346
  - Mainnet
    - Intent: Claiming a bridgeToken call sent from L1 with a depositCount of 177400
      - https://rpc.xlayer.tech/priapi/v1/ob/bridge/merkle-proof?net_id=0&deposit_cnt=177400
    - Intent: Claiming a bridgeToken call sent from L2 with a depositCount of 12
...
2. Description
  - `mainnetFlag`
    - Boolean to represent whether the message originated from L1
  - `rollUpIndex`
