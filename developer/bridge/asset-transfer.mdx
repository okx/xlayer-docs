...
- Bridge smart contract ([ZkEVMBridge.sol](https://github.com/okx/Xgon-contracts/blob/release/v0.1.0/contracts/PolygonZkEVMBridge.sol "ZkEVMBridge.sol"))
- Global Exit Root Manager ([ZkEVMGlobalExitRoot.sol](https://github.com/okx/Xgon-contracts/blob/release/v0.1.0/contracts/PolygonZkEVMGlobalExitRoot.sol "ZkEVMGlobalExitRoot.sol"))
- Consensus contract ([ZkEVM.sol](https://github.com/okx/Xgon-contracts/blob/release/v0.1.0/contracts/PolygonZkEVM.sol "ZkEVM.sol"))
...
1. User `Deposit` transfers assets L2 -> L1 or  L1 -> L2. The bridge smart contract SC adds an exit leaf to the L2 exit tree, which holds the relevant transfer data. 
2. Subsequently, the root of the L2 Exit Tree is updated and becomes accessible for the next smart contract in the workflow, known as the Global Exit Root Manager SC.
3. User `Claim` assets on opposite chain bridge SC.

<Tip title="Note">There are two bridge smart contracts: ‌one deployed in L1, while the other is deployed in L2, both using the same ZkEVMBridge.sol code file, but using different parameters for the interaction.</Tip>
...
<Tip title="Note">There are two Global Exit Root manager SCs: one deployed in L1, while the other is deployed in L2. Their Solidity code files are; ZkEVMGlobalExitRoot.sol, and ZkEVMGlobalExitRootL2.sol, respectively.</Tip>

## Consensus contract
The `ZkEVM.sol` smart contract is the consensus algorithm used in Xgon. It is located in L1 for batch verification purposes.
The consensus smart contract (ZkEVM.sol) has two main functions:
- `sequenceBatches`
- `verifyBatchesTrustedAggregator`

With these functions, L2 tx data can be rolled up and verified. 
1. Transactions are grouped into batches by `sequenceBatches` function.
...
The following diagram illustrates the interactions among the three bridge-related smart contracts when assets are bridged. It describes the overall process of transferring assets from L1 to L2 and from L2 to L1 through contracts.
...
1. The consensus contract uses the `SequenceBatches` function to sequence batches, which includes, among other transactions, asset transfer information.
...
### Specific interaction in L2
The focus in this subsection is on the bridge-related smart contracts used in L2, especially in the zkEVM.
1. The bridge SC (`ZkEVMBridge.sol`) in L2 is the same smart contract deployed on L1. 
...
![L2 interactions with ZK-circuit](./img/l2-interactions-with-zk-circuit.png)
