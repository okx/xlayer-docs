...
- Significantly higher scalability, since rollups’ throughput is bounded by the amount of transaction data that can be published to Ethereum. X Layer uses the OKB token as it's native gas fees. Getting started with X Layer is straightforward, you can by funding your Web3 wallet (we support many!), or by bridging over funds.
