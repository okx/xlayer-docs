...
## Data flow in X1
Transactions on the X1 network have a typical flow starting from the users' interactions with their wallet on X1, to how the transactions are batched, sequenced, and aggregated on Ethereum L1:
1. Transactions are submitted to X1.
...
Once the transaction is considered valid, the sequencer will update the X1 state. Users will experience transactions going through almost instantly.
From here onwards, the user remains engaged in the X1 L2 state. Everything that follows involves sharing transaction data back to Ethereum L1, but this step is only important for users who wish to transfer their funds from X1 L2 back to Ethereum L1.
...
### 6. Reading events on X1
Finally, DApps read information from X1 through the synchronizer.
...
This way, DApps can easily get a view of the state of X1 via JSON-RPC.
