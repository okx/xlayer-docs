# Batch sequencing
Once we have batches of transactions, they are ready to be sequenced. The sequencer will call the L1 `ZkEVM.sol` contract's `sequencedBatches` function and provide it with multiple batches of transactions. It is basically a storage structure that holds the queue of sequences defining the virtual state.
```solidity
...
- It must include a `globalExitRoot` value that is present in the **GlobalExitRootMap** of the bridge's L1 `ZkEVMGlobalExitRoot.sol` contract. A batch is valid only if it includes a valid `globalExitRoot`.
