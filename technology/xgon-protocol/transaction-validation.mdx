...
Once the batches have been successfully aggregated on L1, all zkEVM nodes can verify their local L2 state by directly retrieving and validating consolidated roots from the L1 Consensus Contract (`ZkEVM.sol`). This marks the achievement of a consolidated L2 state.
